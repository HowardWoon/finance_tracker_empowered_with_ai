...
def update_markets(csv_filename: str = "markets.csv", batch_size: int = 500) -> None:
    """
    ...
    current_offset: int = count_csv_lines(csv_filename)
    file_exists: bool = os.path.exists(csv_filename) and current_offset > 0
    
    if file_exists:
        logger.info(f"Found {current_offset} existing records. Resuming from offset {current_offset}")
        mode = 'a'
    else:
        logger.info(f"Creating new CSV file: {csv_filename}")
        mode = 'w'
        ...
        while True:
            ...
            try:
                ...
                if response.status_code == 500:
                    logger.warning(f"Server error (500) - retrying in 5 seconds... (attempt {retry_count+1}/{max_retries})")
                    if retry_count < max_retries:
                        retry_count += 1
                        time.sleep(5)
                        continue
                    else:
                        logger.error("Max retries exceeded for 500 error")
                        break
                        
                elif response.status_code == 429:
                    logger.warning(f"Rate limited (429) - waiting 10 seconds...")
                    time.sleep(10)
                    continue
                    ...
                    if retry_count < max_retries:
                        retry_count += 1
                        logger.info(f"Retrying in 3 seconds... (attempt {retry_count}/{max_retries})")
                        time.sleep(3)
                        continue
                    else:
                        break
                ...
                markets: List[Dict] = response.json()
                
                if not markets:
                    logger.info(f"No more markets found at offset {current_offset}. Completed!")
                    break

...
try:
    ...
    print("Please ensure gql is installed: pip install gql[requests]>=3.4.0")
    sys.exit(1)
...
try:
    ...
    logger.warning(f"Could not import update_markets: {e}")
...
def save_cursor(timestamp: int, last_id: Optional[str], sticky_timestamp: Optional[int] = None) -> None:
    ...
    with open(CURSOR_FILE, 'w') as f:
        json.dump(state, f)
    logger.debug(f"Cursor saved: timestamp={timestamp}, last_id={last_id}, sticky={sticky_timestamp}")

def get_latest_cursor() -> Tuple[int, Optional[str], Optional[int]]:
